...
function searchSortedArray(
  ...
  if (array[middleIndex] === number) { // O(log n)
    return middleIndex;
...
//The time compelxity is logrithmic time O(log n) because
//The space complexity is ??? becuase it's a recursive operation.
...
//The time complexity is quadratic time, O(n^2) because there's a nested loop.
//The space complexity is linear because it returns an array.
...
//The space complexity is linear because the final value returns an array.
...
//The time complexity is O(n^2) because the includes method takes O(n) and since it's inside of forEach method which also takes O(n), the consolidated time is quadratic.
// The space complexity is linear bacause it returns an array. 
